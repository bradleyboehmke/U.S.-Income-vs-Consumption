...
selectInput("state", "Select State:", unique(data_clean$Location),
            selected = "Ohio")
...
### Underlying Data {data-width=400}
...
This project involves exploring [Bureau of Economic Analysis](http://www.bea.gov/index.htm) data regarding [per capita disposable income]() (hereafter referred to as PCI) and [per capita personal expenditures]() (hereafter referred to as PCE). The PCI data provides annual (non-inflation adjusted) per capita disposable income at the national and state-level from 1948-2015 and the PCE data provides annual (non-inflation adjusted) per capita personal consumption expenditures at the national and state-level from 1997-2014. Consequently, this research looks at how state-level savings rates defined as $Savings = PCI - PCE$ has changed over time and by geographic location.
